...
@section('main-content')
    <div class="card card-warning">
        ...
        <div class="card-body">
            <form action="{{ $nuancier->exists ? route('nuancierFacade.update', $nuancier) : route('nuancierFacade.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                @method($nuancier->exists ? 'PUT' : 'POST')

                <div class="row">
                    <div class="col-sm-6">
                        <div class="form-group">
                            <input type="text" class="form-control is-valid" name="title" id="title" placeholder="Nom ..." value="{{ $nuancier->exists ? $nuancier->title : old('title') }}">
                        </div>
                    </div>
                    <div class="col-sm-6">
                        <div class="el-select2">
                            <select id="category_id" class="form-control el-select" name="category_id">
                                <option value="">Catégorie</option>
                                @foreach($categories as $category)
                                    @if($nuancier->exists && $nuancier->category && $nuancier->category->id === $category->id)
                ...
                @if($nuancier->exists)
                    @if($nuancier->document)
                        <div class="el-container-grid-column-document">
                            <div id="el-container-picture" style="width: 100%;">
                                <div class="el-container">
                                    <img src="{{ asset($nuancier->document->path) }}" alt="..." class="img-thumbnail">
                                    <button class="el-remove"
                                            hx-delete="{{ route('document.destroy', ['document' => $nuancier->document, 'isMultiple' => 0]) }}"
                ...
                <div class="row">
                    <div class="col-12">
                        <textarea id="summernote" name="content"> {{ $nuancier->content ?? old('content') }} </textarea>
                    </div>
                </div>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const pond = FilePond.create(inputElement, {
                    ...
                    server: {
                        process: {
                            url: '{{ route("nuancierFacade.uploadDocument", $nuancier) }}', // Remplacez par l'URL de votre action Laravel pour l'upload
                            method: 'POST',
