...
@section('main-content')
    <div class="card card-warning">
        ...
        <div class="card-body">
            <form action="{{ $caracteristic->exists ? route('caracteristic.update', $caracteristic) : route('caracteristic.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                @method($caracteristic->exists ? 'PUT' : 'POST')
                        ...
                        <div class="el-select2">
                            <select id="article_id" name="article_id"  class="form-control el-select">
                                ...
                                @foreach($articles as $article)
                                    @if($caracteristic->exists && ($caracteristic->article->id === $article->id || $caracteristic->article->id === old('article_id')) )
                ...
                @if($caracteristic->exists)
                    @if($caracteristic->document)
                        <div class="el-container-grid-column-document">
                            <div id="el-container-picture" style="width: 100%;">
                                <div class="el-container">
                                    <img src="{{ asset($caracteristic->document->path) }}" alt="..." class="img-thumbnail">
                                    <button class="el-remove"
                                            hx-delete="{{ route('document.destroy', ['document' => $caracteristic->document, 'isMultiple' => 0]) }}"
                ...
                <div class="row">
                    <div class="col-12">
                        <textarea id="summernote" name="content"> {{ $caracteristic->content ?? old('content') }} </textarea>
                    </div>
                </div>
