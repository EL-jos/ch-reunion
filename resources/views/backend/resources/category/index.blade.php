                    ...
                    <table id="example1" class="table table-bordered table-striped">
                        ...
                        <tbody>
                            @foreach($categories as $category)
                                <tr>
                                    <td>{{ $loop->index  + 1}}</td>
                                    {{--<td> <img height="71px" src="{{ $brand->document()->where('type','image')->first() ? asset($brand->document()->where('type','image')->first()->path) : '' }}"></td>--}}
                                    <td>{{ $category->name }}</td>
                                    <td>
                                        @if(!$category->trashed())
                                            <div class="d-flex" style="gap: .5rem;">
                                                <a href="{{ route('category.edit', $category) }}" class="btn bg-gradient-warning"><i class="fas fa-edit"></i></a>
                                                <button onclick="document.getElementById('el-delete-form-category-{{ $category->id }}').submit()" class="btn bg-gradient-danger">
                                                    <form id="el-delete-form-category-{{ $category->id }}" method="post" action="{{ route('category.destroy', $category) }}">
                                                        @csrf
                                                        @method("DELETE")
                                                    </form>
                                        ...
                                        @else
                                            <button onclick="document.getElementById('el-restore-form-{{ $category->id }}').submit()" class="btn bg-gradient-warning">
                                            ...
                                            <button class="btn bg-gradient-danger" onclick="document.getElementById('el-form-{{$category->id}}').submit()">
                                                <form id="el-form-{{$category->id}}" action="{{ route('category.forceDelete', $category) }}" method="post">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                                <i class="fas fa-trash-alt"></i>
